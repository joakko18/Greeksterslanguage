...
export default function LanguageSwitcher({ lng }: LanguageSwitcherProps) {
  ...
  return (
    <div className="flex space-x-2 bg-white/30 backdrop-blur-sm rounded-full p-1 shadow-lg 
                sm:p-1 md:p-2"> {/* adjust padding for mobile if needed */}
  <Link
    href={getPathname('en')}
  ...
  <Link
    href={getPathname('gr')}
    className={`px-2 py-0.5 rounded-full text-xs sm:text-sm font-semibold transition-colors duration-200 
                bg-center bg-cover bg-[url('https://res.cloudinary.com/desem7vhd/image/upload/v1756573079/ee_i4enfz.png')] ${
                  lng === 'gr' ? 'bg-green-300 text-white' : 'text-gray-950 font-bold hover:bg-gray-100'
                }`}
  >
    GR
  </Link>
